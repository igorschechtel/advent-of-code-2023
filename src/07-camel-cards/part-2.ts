...
const getCardStrength = (card: string): number => {
  ...
  const numJokers = occurrencesMap['J'] || 0;
  const occurrences = Object.entries(occurrencesMap)
    .filter(([card]) => card !== 'J')
    .map(([, count]) => count);

  const maxIndex = occurrences.indexOf(Math.max(...occurrences));
  occurrences[maxIndex] += numJokers;
